# Pi workstation + VS Code

A schema-v2 mixin that adds browser-based **code-server** (container port 8080)
and the Microsoft **VS Code CLI** tunnel to the `pi` agent. It was split out
of the `pi` kit so that kit stays fast to launch — the two VS Code downloads
...
```sh
sbx run \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=pi' \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=pi-code' \
  pi
```
