...
function renderEvents() {
  ...
  for (const event of events) {
    ...
    summary.textContent =
      event.summary || event.payload?.transcript_delta || "";
    ...
    star.addEventListener("click", async () =>
      patchEvent(event.id, { starred: !event.starred, note: note.value }),
    );
...
async function patchEvent(id, patch) {
  const updated = await getJson(`/api/events/${id}`, {
    method: "PATCH",
...
searchEl.addEventListener("input", () => {
  ...
  searchEl.timer = setTimeout(loadEvents, 250);
});
