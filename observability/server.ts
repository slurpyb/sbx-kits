...
const dataDir = process.env.OBSERVABILITY_DATA_DIR ?? join(import.meta.dir, "data");
...
db.run("CREATE INDEX IF NOT EXISTS idx_events_session ON events(session_id, id DESC)");
db.run("CREATE INDEX IF NOT EXISTS idx_events_received ON events(received_at DESC)");
db.run("CREATE INDEX IF NOT EXISTS idx_events_starred ON events(starred, id DESC)");
...
const insertEvent = db.query(`
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
const updateEvent = db.query("UPDATE events SET starred = ?, note = ? WHERE id = ?");
...
function summarize(body: JsonObject, payload: JsonObject): string {
  const direct = text(payload.prompt)
  ...
  const tool = text(payload.tool_name) ?? text(payload.toolName) ?? text(toolCall.name);
...
function normalize(body: JsonObject) {
  const payload = Object.keys(object(body.payload)).length > 0 ? object(body.payload) : body;
...
function rowToJson(row: JsonObject | null): JsonObject | null {
  if (!row) return row;
  let payload: unknown = {};
  try { payload = JSON.parse(String(row.payload)); } catch { payload = { malformed: true, raw: String(row.payload) }; }
  return {
    ...row,
...
function broadcast(event: unknown) {
  ...
  for (const client of [...streamClients]) {
    try { client.enqueue(message); } catch { streamClients.delete(client); }
  }
}

async function ingest(req: Request): Promise<Response> {
  let body: JsonObject;
  try { body = object(await req.json()); } catch { return json({ error: "invalid JSON" }, 400); }
  const event = normalize(body);
  const result = insertEvent.run(event.receivedAt, event.emittedAt ?? null, event.harness, event.sessionId, event.eventName, event.cwd ?? null, event.model ?? null, event.summary, event.payload);
  const saved = rowToJson(selectEvent.get(Number(result.lastInsertRowid)) as JsonObject | null);
...
function listEvents(url: URL): Response {
  ...
  if (q) { where.push("(summary LIKE ? OR payload LIKE ? OR note LIKE ?)"); args.push(`%${q}%`, `%${q}%`, `%${q}%`); }
...
async function patchEvent(req: Request, id: number): Promise<Response> {
  ...
  try { patch = object(await req.json()); } catch { return json({ error: "invalid JSON" }, 400); }
  const starred = typeof patch.starred === "boolean" ? Number(patch.starred) : Number(current.starred);
  const note = typeof patch.note === "string" ? patch.note.slice(0, 4000) : String(current.note ?? "");
...
function eventStream(): Response {
  ...
  const stream = new ReadableStream<string>({
    ...
    cancel() { if (controllerRef) streamClients.delete(controllerRef); },
  });
...
async function route(req: Request): Promise<Response> {
  let url: URL;
  try { url = new URL(req.url); } catch { return json({ error: "invalid URL" }, 400); }
  ...
  if (eventMatch && req.method === "PATCH") return patchEvent(req, Number(eventMatch[1]));
  if (url.pathname === "/stream") return eventStream();
  if (url.pathname === "/app.js") return new Response(Bun.file(join(import.meta.dir, "app.js")), { headers: { "Content-Type": "text/javascript" } });
  if (url.pathname === "/" || url.pathname === "/index.html") return new Response(Bun.file(join(import.meta.dir, "index.html")), { headers: { "Content-Type": "text/html; charset=utf-8" } });
  return json({ error: "not found" }, 404);
}

const server = Bun.serve({ hostname: "0.0.0.0", port, fetch: route });

process.stdout.write(`Agent observability dashboard listening on http://0.0.0.0:${server.port}\n`);
