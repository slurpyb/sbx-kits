# Antigravity workstation + VS Code

A schema-v2 mixin that adds browser-based **code-server** (container port 8080)
and the Microsoft **VS Code CLI** tunnel to the `gemini` agent. It was split out
of the `agy-cli` kit so that kit stays fast to launch — the two VS Code downloads
...
```sh
sbx run \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=agy-cli' \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=agy-cli-code' \
  gemini
```
