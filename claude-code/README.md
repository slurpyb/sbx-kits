# Claude workstation + VS Code

A schema-v2 mixin that adds browser-based **code-server** (container port 8080)
and the Microsoft **VS Code CLI** tunnel to the `claude` agent. It was split out
of the `claude-workstation` kit so that kit stays fast to launch — the two VS Code downloads
...
```sh
sbx run \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=claude-workstation' \
  --kit 'git+https://github.com/slurpyb/sbx-kits.git#ref=<commit-sha>&dir=claude-code' \
  claude
```
